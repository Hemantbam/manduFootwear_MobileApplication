...
export const shoeDetailsValidationSchema = Joi.object({
  brandName: Joi.string().trim().required().min(3).max(30).messages({
  ...
  category: Joi.string().optional().max(50).messages({
    "string.base": "Category must be a string",
    "string.max": "Category must not exceed 50 characters",
  }),

  material: Joi.string().optional().max(100).messages({
  ...
  availableStock: Joi.number().required().messages({
